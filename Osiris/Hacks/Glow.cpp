...
void Glow::render() noexcept
{
    ...
    for (int i = 0; i < memory->glowObjectManager->glowObjectDefinitions.size; i++) {
        ...
        auto applyGlow = [&glowobject](decltype(glow[0])& glow, int health = 0, Entity* entity = nullptr) noexcept
        {
            if (glow.enabled || (entity && entity != nullptr && !entity->isDormant() && entity->isAlive() && entity->isFlashed() && config->visuals.grenadeBeams.enabled)) {
        ...
        auto applyPlayerGlow = [applyGlow](decltype(glow[0])& glowAll, decltype(glow[0])& glowVisible, decltype(glow[0])& glowOccluded, Entity* entity) noexcept {
            if (glowAll.enabled || (entity->isFlashed() && config->visuals.grenadeBeams.enabled)) applyGlow(glowAll, entity->health(), entity);
            else if (entity->visibleTo(localPlayer.get())) applyGlow(glowVisible, entity->health(), entity);
            else applyGlow(glowOccluded, entity->health(), entity);
        };
        ...
        case ClassId::CSPlayer:
            if (auto activeWeapon{ entity->getActiveWeapon() }; activeWeapon && activeWeapon->getClientClass()->classId == ClassId::C4 && activeWeapon->c4StartedArming())
                ...
                applyGlow(glow[12], entity->health(), entity);
