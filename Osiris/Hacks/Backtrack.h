...
namespace Backtrack {
    ...
    static void init() noexcept
    {
        ...
        if (records && records->size()) {
            exists = true;
            for (std::deque<Backtrack::Record> record : records) {

                if (record.size() && Backtrack::valid(record.front().simulationTime)) {
                    records_backup[i] = record.front();
                }
                else {
                    records_backup[i] = invalid;
                }
                i++;
                if (i >= 64)
                    break;
        ...
        */
        ...
        if (exists) {
            for (auto record : records_backup) {
                if (record.invalid) {
                    continue;
                }
