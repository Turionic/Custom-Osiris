...
bool HandleBulletPenetration(WeaponInfo* wpn_data, FireBulletData& data, bool extracheck, Vector& wallbangPos, float minDamage) noexcept;
...
bool HandleBulletPenetration(WeaponInfo* wpn_data, FireBulletData& data, bool extracheck, Vector& wallbangPos, float minDamage = 0.0f)noexcept
{
...
float AutoWall::Damage(const Vector& point, Vector& wallbangVector, float minDamage = 0.0f)noexcept
{
...
float AutoWall::EntityDamage(Entity* entity, const Vector& point) noexcept
{
	auto data = FireBulletData(entity->getEyePosition(), entity);
...
bool AutoWall::CanHitFloatingPoint(const Vector& point, const Vector& source, Vector& wallbangPos)noexcept
{
	...
	if (HandleBulletPenetration(weaponData, data, extra_check, wallbangPos, 0.0f))
	{
		return true;
	}
...
bool AutoWall::CanWallbang(float& dmg, Vector& wallbangPos)noexcept
{
	...
	if (HandleBulletPenetration(weaponData, data, false, wallbangPos, 0.0f))
	{
