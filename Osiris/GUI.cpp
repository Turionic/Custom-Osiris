...
void GUI::renderRenderablesWindow(bool contentOnly) noexcept {
    if (!contentOnly) {
        if (!window.renderables)
            return;

        ImGui::SetNextWindowSize({ 800.0f, 0.0f });
        ImGui::Begin("Renderables", &window.renderables, windowFlags);
    }
    ImGui::Columns(2);
    ImGui::SetColumnWidth(0, 400.0f);
    ImGui::SetColumnWidth(1, 400.0f);
    ImGui::Text("--- SDK/Beams --- ");
    ImGuiCustom::colorPicker("Grenade Beams", config->visuals.grenadeBeams);
    ImGuiCustom::colorPicker("Grenade Explosion Rings", config->visuals.grenadeBounce);
    ...
    ImGui::Text(" --- Color/Alpha Material Modulation --- ");
    ImGuiCustom::colorPicker("All color", config->visuals.all);
    ImGuiCustom::colorPicker("World color", config->visuals.world);
    ImGuiCustom::colorPicker("World (PreCached) color", config->visuals.PrecacheWorld);
    ImGuiCustom::colorPicker("World (Non-Cached) color", config->visuals.NonCachedWorld);
    ImGuiCustom::colorPicker("Sky color", config->visuals.sky);
    ImGuiCustom::colorPicker("Static Prop (All) color", config->visuals.props);
    ImGuiCustom::colorPicker("Static Prop (PreCached) color", config->visuals.PrecacheProps);
    ImGuiCustom::colorPicker("Static Prop (Non-Cached) color", config->visuals.NonCached);
    ImGui::Checkbox("Ignore Precache'd Materials (Transparency)", &config->visuals.Ignore_precache);
    ImGui::Checkbox("Precache'd Materials Only (Transparency)", &config->visuals.Precache_Only);
    ImGui::PushID(0);
    ImGui::SliderFloat("", &config->visuals.alphaval, 0.0f, 1000.0f, "World Transparency: %.2f");
    ImGui::PopID();

    ImGui::SameLine();
    ImGui::Checkbox("", &config->visuals.worldtrans);
    ImGui::PopID();
    ImGui::PushID(1);
    ImGui::SliderFloat("", &config->visuals.propalphaval, 0.0f, 1000.0f, "Prop Transparency: %.2f");
    ImGui::PopID();


    ImGui::SameLine();
    ImGui::PushID(2);
    ImGui::Checkbox("", &config->visuals.proptrans);
    ImGui::PopID();
    ImGui::NextColumn();
    ...
    ImGui::PopID();
    ImGui::PushID(4);
    ImGui::SliderFloat("", &config->visuals.smokealpha, 0.0f, 1000.0f, "Smoke Transparency: %.2f");
    ImGui::PopID();

    ImGui::SameLine();
    ImGui::PushID(3);
    ImGui::Checkbox("", &config->visuals.smoketrans);
    ImGui::PopID();
    ImGui::PushID(5);
    ImGui::SliderInt("", &config->visuals.customflag, 0, 32, "Smoke Custom Flag: %d (Warning: Dangerous)");
    ImGui::PopID();
    ImGui::SameLine();
    ImGui::PushID(6);
    ImGui::Checkbox("", &config->visuals.smokeflagmanual);
    ImGui::PopID();
...
void GUI::renderDebugWindow(bool contentOnly) noexcept 
{
    if (!contentOnly) {
        ...
        ImGui::SetNextWindowSize({ 1200.0f, 0.0f });
        ImGui::Begin("Debug", &window.debug, windowFlags);
    }
...
void GUI::renderAntiAimWindow(bool contentOnly) noexcept
{
    ...
    if (config->antiAim.general.fakeWalk.enabled) {
        ...
        ImGui::SliderInt("Speed", &config->antiAim.general.fakeWalk.maxChoke, 1, 15);
    }
    ImGui::Text("Swap Desync Side Key: ");
    ImGui::SameLine();
    hotkey(config->antiAim.swapsidekey);
    /*
    ImGui::Checkbox("Force Move Fix", &config->antiAim.forceMovefix);
    ImGui::Checkbox("Force Move Fix Post", &config->antiAim.forceMovefixPost);
    ImGui::Checkbox("Force Move Fix On Send", &config->antiAim.forceMovefixSend);
    ImGui::Checkbox("Force Move Fix On !Send", &config->antiAim.forceMovefixNoSend);
    ImGui::Checkbox("Secret Debug Shit", &config->antiAim.secretdebug);
    ImGui::SliderFloat("LBY Amount", &config->antiAim.LBYValue, -360, 360);
    ImGui::Checkbox("Spin", &config->antiAim.Spin);
    */
    ImGui::Checkbox("To Wall", &config->antiAim.toWall);
    ImGui::Checkbox("Random shit", &config->antiAim.swapsidesspam);
    ImGui::Checkbox("KeyYaw", &config->antiAim.KeyYaw);

    ImGui::TextUnformatted("BackWard Yaw Key");
    ImGui::SameLine();
    hotkey(config->antiAim.BackwardYKey);

    ImGui::TextUnformatted("Left Yaw Key");
    ImGui::SameLine();
    hotkey(config->antiAim.LeftYKey);

    ImGui::TextUnformatted("Right Yaw Key");
    ImGui::SameLine();
    hotkey(config->antiAim.RightYKey);

    ImGui::TextUnformatted("Forward Yaw Key");
    ImGui::SameLine();
    hotkey(config->antiAim.ForwardYKey);
    /*
    ...
    */
    ...
    /*
    ImGui::SliderFloat("Subtractor", &config->antiAim.subtractor, -116.0f, 116.0f);
    ImGui::SliderFloat("Last lby", &config->antiAim.lastlby, -116.0f, 116.0f);
    ImGui::SliderFloat("Last lby val", &config->antiAim.lastlbyval, -116.0f, 116.0f);
    */
    if (!contentOnly)
        ImGui::End();
}
...
void GUI::renderBacktrackWindow(bool contentOnly) noexcept
{
    ...
    ImGui::Checkbox("Enabled", &config->backtrack.enabled);
    ImGui::SameLine();
    ImGui::Checkbox("Extend with fake ping (debug button)", &config->backtrack.fakeLatency);
    //ImGui::SameLine();
    ImGui::Checkbox("Fake Latency On key", &config->backtrack.onKey);
    hotkey(config->backtrack.fakeLatencyKey);
    ImGui::Checkbox("Ignore smoke", &config->backtrack.ignoreSmoke);
    ImGui::SameLine();
    ImGui::Checkbox("Recoil based fov", &config->backtrack.recoilBasedFov);

    int TimeLimitRoof = 200; if (config->backtrack.fakeLatency) { TimeLimitRoof = 1000; }
    ...
    ImGui::SliderInt("", &config->backtrack.timeLimit, 1, TimeLimitRoof, "Time Limit %d ms (above 200; Use Fake Ping)");
    ImGui::PopID(); ImGui::PopItemWidth();
    ImGui::Checkbox("Draw All Backtrack Ticks in Chams", &config->backtrack.backtrackAll);
    ImGui::SliderInt("BackTrack Draw Step Size", &config->backtrack.step, 1, 24, "%d");
    ImGui::Checkbox("Bread Crumbs (Extended Backtrack Records for Chams)", &config->backtrack.extendedrecords);
    ImGui::SliderFloat("Breadcrumb Creation Time", &config->backtrack.breadcrumbtime, 200.0f, 5000.0f, "%.2f ms");
    ImGui::SliderFloat("Breadcrumb Fade Rate", &config->backtrack.breadexisttime, 0.0f, 100.0f, "%.2f ms");
...
void GUI::renderStreamProofESPWindow(bool contentOnly) noexcept
{
    ...
    if (ImGui::BeginChild("##child", { 400.0f, 0.0f })) {
        ...
        if (currentCategory < 2) {
            ...
            ImGui::Checkbox("Draw Multipoints", &playerConfig.drawMultiPoints);
            ...
            if (ImGui::BeginPopup("")) {
                ...
                trailPicker("Local Player Extra", trails.localPlayerextra);
...
void GUI::renderMiscWindow(bool contentOnly) noexcept
{
    ...
    ImGui::SliderFloat("Trigger Velocity", &config->misc.fakelagspeed, 0.0f, 255.0f);
    ImGui::Checkbox("Peek Lag", &config->misc.peeklag);
    ...
    ImGui::Checkbox("Walkbot v1", &config->misc.walkbot);
    ImGui::SliderInt("Wakbot Speed", &config->misc.walkbotspeed, 0, 255);
        ...
        /*
            ...
            //bool Choke{ false };
            int psState{ 0 };
            float PrevVeloF{ 0.0f };
            float PrevVeloS{ 0.0f };
        */

        ImGui::Checkbox("Perfect Shot", &config->misc.perfectShot);
        ImGui::Checkbox("Counter on Shot", &config->misc.Counter);
        ImGui::SameLine();
